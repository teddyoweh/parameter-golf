    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if _HAS_FA3:
            ...
            if self.num_kv_heads != self.num_heads:
                ...
                B_s, _, T_s, D_s = k_sdpa.shape
                k_sdpa = k_sdpa[:, :, None, :, :].expand(B_s, self.num_kv_heads, reps, T_s, D_s).reshape(B_s, self.num_heads, T_s, D_s)
                v_sdpa = v_sdpa[:, :, None, :, :].expand(B_s, self.num_kv_heads, reps, T_s, D_s).reshape(B_s, self.num_heads, T_s, D_s)
...
class TrigramHashEmbedding(nn.Module):
    """3-gram hash embedding. Captures trigram context beyond BigramHash."""
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_optimizer == "adamw":
        # AdamW with per-layer LR groups: MLP down/proj get 3x, MLP up/fc get 0.5x
        proj_p, fc_p, other_p = [], [], []
        for name, p in base_model.named_parameters():
        ...
        optimizer = torch.optim.AdamW([
            {"params": proj_p, "lr": args.ttt_lr * 3.0, "initial_lr": args.ttt_lr * 3.0},
            {"params": fc_p, "lr": args.ttt_lr * 0.5, "initial_lr": args.ttt_lr * 0.5},
            {"params": other_p, "lr": args.ttt_lr, "initial_lr": args.ttt_lr},
        ], weight_decay=0.0)
        log0(f"ttt_sliding:optimizer=AdamW lr={args.ttt_lr} proj_lr={args.ttt_lr*3} fc_lr={args.ttt_lr*0.5}")
    else:
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                cos_mul = 0.5 * (1.0 + math.cos(math.pi * ci / max(num_chunks - 1, 1)))
                for pg in optimizer.param_groups:
                    base_lr = pg.get('initial_lr', args.ttt_lr)
                    pg['lr'] = base_lr * cos_mul
...
def main() -> None:
    ...
    # FA3 handles attention directly, disable all SDPA backends
    ...
    from torch.backends.cuda import flash_sdp_enabled, cudnn_sdp_enabled, mem_efficient_sdp_enabled, math_sdp_enabled
    log0(f"sdp_backends:cudnn={cudnn_sdp_enabled()} flash={flash_sdp_enabled()} mem_efficient={mem_efficient_sdp_enabled()} math={math_sdp_enabled()}")
